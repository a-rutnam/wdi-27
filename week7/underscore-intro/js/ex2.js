...
const grouped = _.groupBy(words, w => w[0].toLowerCase() );
console.log(grouped);

// Check to see if all the words have more than 3 characters
const allMoreThanThree = _.every(words, w => w.length > 3 );
console.log('all more than three?', allMoreThanThree);

// Check if some words are over twenty characters long
const anyOverTwenty = _.some(words, w => w.length > 20 );
console.log('any over twenty?', anyOverTwenty);

// Get an array of all of the uids in people
console.log( 'uids:', _.pluck(people, 'uid') );

// Find the person with the highest uid
console.log( 'max:', _.max(people, 'uid') );

// Return an object that says how many even numbers and how many odd  numbers there are in numbers
const groupedEvenOdd = _.countBy(numbers, n => n % 2 === 0 ? 'even' : 'odd' );
console.log('even and odd:', groupedEvenOdd);
